...
int main(int argc, char *argv[])
{
	try {
		...
		*nsweeps = sched.size();
        ...
        std::vector< std::vector< Algorithm<>::site_type > > vec_sites;
		...
		for (std::size_t rep = *rep0; rep < *nreps + *rep0; ++rep) {
            ...
            auto new_sites = alg.get_sites();
            if ((en[offs - 1] < min_en) || (rep == *rep0)) {
                min_en = en[offs - 1];

                vec_sites.clear();
                vec_sites.push_back(new_sites);
			} else if (en[offs - 1] == min_en) {
                bool is_equal = false;
                for (auto sites : vec_sites) {
                    is_equal = true;
                    for (unsigned int i = 0; i < sites.size(); i++) {
                        if (new_sites[i].key != Algorithm<>::index_type(-1)) {
                            is_equal &= (new_sites[i].spin == sites[i].spin);
                        }
                    }
                    if (is_equal) {
                        break;
		...
		for (auto sites : vec_sites) {
            output = "{";
		    for (auto site: sites) {
		        if (site.key != Algorithm<>::index_type(-1)) {
                    output += std::to_string(site.key) + ": " + std::to_string(site.spin) + ", ";
                }
		    }
		    output.pop_back();
		    output.pop_back();
